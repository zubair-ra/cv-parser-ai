  ...
  validateProvider(provider) {
    ...
    if (!availableProviders.includes(provider)) {
      ...
      throw new CVParserError(
        `Provider '${provider}' is not supported or not installed. ` +
        `Available providers: ${supportedList}. ` +
        `Install missing providers with: npm install openai @anthropic-ai/sdk`
      );
    }
  }
  ...
  async _postProcessResults(data, extractedData, options) {
    ...
    if (options.includeMetadata) {
      ...
      processedData.metadata = {
        ...
        processingTime: Date.now(), // Can be enhanced to track actual time
        ...(processedData.metadata || {})
      };
    ...
    const confidence = processedData.metadata?.parseConfidence || 0;
